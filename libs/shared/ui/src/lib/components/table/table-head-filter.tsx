...
import {
  ...
  type TableHeadCustomFilterProps,
  type TableHeadProps,
} from './table'
import { MenuItemProps } from '../menu/menu-item'
import { Icon } from '../icons/icon'
import { upperCaseFirstLetter } from '@placecare/utils'
...
export function groupBy<T>(
  ...
  if (dataHeadFilter?.itemsCustom) {
    ...
    const result: MenuItemProps[] = [
      ...
      contentLeft: (
        <Icon
          ...
          className={`text-sm ${
            currentFilter === item ? 'text-green-400' : 'text-transparent'
          }`}
        />
      ),
      onClick: () => {
    ...
    const result: MenuItemProps[] = Object.keys(dataByKeys).map(
      (key: string) => ({
        ...
        contentLeft: (
          <Icon
            name={'material-symbols:check'}
            className={`text-sm ${
              currentFilter === key ? 'text-green-400' : 'text-transparent'
            }`}
          />
        ),
        contentRight: (
        ...
        itemContentCustom:
          dataHeadFilter?.itemContentCustom &&
          dataHeadFilter?.itemContentCustom(dataByKeys[key][0], currentFilter),
        onClick: () => {
...
export function TableHeadFilter<T>({
  ...
  const query = dataHead.filter?.[0].query || ''
  ...
  useEffect(() => {
    filter.find(
      (item) =>
        item.key === key &&
        item.value !== ALL &&
        setCurrentFilter(item.value || ALL)
    )
  }, [filter])
  ...
  function cleanFilter(event: MouseEvent) {
    ...
    setFilter &&
      setFilter((prev) => {
        ...
        return [...result, { key: key, value: ALL, query: query }]
      })
  }
  ...
  const hideFilterNumber: boolean =
    dataHead.filter?.some((item) => item.hideFilterNumber) || false
  const hasFilter = filter?.some(
    (item) => item.key === dataHead.filter?.[0].key && item.value !== ALL
  )
  ...
  return (
    <div className="flex items-center">
      <Menu
        ...
        trigger={
          <div className="flex">
            {hasFilter ? (
              <Button
                className="whitespace-nowrap flex btn--active !h-6 !pr-[26px]"
                size={ButtonSize.TINY}
              >
                {currentFilter} {!hideFilterNumber ? `(${dataFilterNumber})` : ''}
              </Button>
            ) : (
              <Button
                ...
                iconRight={'material:keyboard_arrow_down'}
              >
                {title}
              </Button>
            )}
      ...
      {hasFilter && (
        <span
          role="button"
          className="flex items-center px-2 relative -left-7 mt-0.5 text-neutral-50 text-xs cursor-pointer"
          onClick={(event) => cleanFilter(event)}
        >
          <Icon name={'gravity-ui:xmark'} className="w-5 " />
        </span>
      )}
    </div>
  )
