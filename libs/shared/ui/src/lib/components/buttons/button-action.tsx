import { Menu, MenuAlign, MenuData } from '../menu'
...
export function ButtonAction(props: PropsWithChildren<ButtonActionProps>) {
  ...
  const defineClass = `btn-action btn--${size} ${
    style ? `btn-action--${style}` : ''
  } ${disabled ? 'btn-action--disabled' : ''} ${className}`
    ...
    <div data-testid="button-action" className={defineClass}>
      {menus.length > 0 && (
        <Menu
          ...
          trigger={
            <div
              className={`btn-action__trigger btn-action__trigger--${
