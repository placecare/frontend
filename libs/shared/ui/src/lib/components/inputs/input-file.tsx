...
export function InputFile(props: InputFileProps) {
  ...
  const [selectedImage, setSelectedImage] = useState<
    string | undefined | Blob | MediaSource
  >(value)
  ...
  return (
    <label
      ...
      className={`relative flex items-center justify-center w-full group aspect-video rounded bg-neutral-50 border border-neutral-300 ease-out duration-150 border-dashed cursor-pointer ${
        ...
        <>
          {value instanceof File ? (
            <div className="flex items-center gap-x-2">
              ...
              <p className="text-neutral-400">{value.name}</p>
            </div>
          ) : (
            <img
          ...
          >
            <Icon name={''} className="text-xs" />
          </span>

          <div className="absolute top-0 opacity-0 hover:opacity-100 bg-gray-900/50 backdrop-blur-sm h-full w-full flex justif-center items-center">
            <div
              className="mx-auto items-center flex text-white rounded-md p-2 bg-primary"
              onClick={() => {
                const file = selectedImage as Blob
